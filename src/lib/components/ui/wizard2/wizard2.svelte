<script lang="ts">
  ...
  console.log("Active step next enabled: ", activeStepConfig.nextEnabled);
  ...
  <Dialog.Content>
    <Dialog.Header>
      ...
      <Dialog.Description>
        ...
        <div>
          CURRENT {activeStep} - NEXT? {stepConfigs[0].nextEnabled} - ACTIVE NEXT?
          {activeStepConfig.nextEnabled}
        </div>
            ...
            {#each stepReviews as review, i}
              <li>
                <Collapsible.Root open={true}>
                  <Collapsible.Trigger
                    >{stepConfigs[i].title}</Collapsible.Trigger
                  >
      ...
      <Dialog.Footer class="flex justify-between">
        ...
        <Button onclick={prevStep} variant="outline" disabled={activeStep === 1}
          >Back</Button
        >
        <Button onclick={nextStep} disabled={!activeStepConfig.nextEnabled}
          >Next ({activeStepConfig.nextEnabled})</Button
        >
