...
import { authenticatedUser } from "$lib/stores/auth.store";
...
export const load: LayoutLoad = async ({ data }) => {
  ...
  if (keycloakConfig && keycloakConfig.enabled) {
    console.log(
      "[Microcks launch] Keycloak is enabled, launching OIDC login flow..."
    );
    ...
    if (location.origin.indexOf("/localhost:") != -1) {
      console.log(
        "[Microcks launch] Running locally so disabling Keycloak checkLogin Iframe to respect modern browser restrictions"
      );
    ...
    console.log(
      "[Microcks launch] Keycloak is disabled so running in dev mode with anonymous authent"
    );
    await loadApplicationServices();
  }

  console.log(
    "Layout Authenticated user: " + JSON.stringify(get(authenticatedUser))
  );
};
