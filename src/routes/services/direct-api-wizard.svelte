<script lang="ts">
  ...
  let step1Config: WizardStepConfig = $state({
    id: "step1",
  ...
  function updateApiType(): void {
    step1Config.nextEnabled =
      apiType !== undefined &&
      apiType != null &&
      (apiType == ServiceType.GENERIC_REST ||
        apiType == ServiceType.GENERIC_EVENT);
  ...
  function test() {
    console.log(step1Config.nextEnabled);
  }
</script>
...
{#snippet step3View()}
  <div class="flex flex-col items-start justify-start gap-4 py-4">
    <div class="inline-flex items-center gap-2 font-medium text-lg">
      <fieldset>
        <legend
          ><span class="fa fa-rocket"></span> API reference resource</legend
        >
        <div class="control-group">
          <label class="control-label" for="referencePayload"
  ...
  <form class="form">
    <div class="wizard-pf-review-item">
      ...
      <span class="wizard-pf-review-item-value"
        ><code>{api.referencePayload}</code></span
      >
    </div>
  </form>
{/snippet}
